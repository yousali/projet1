...
import static projet1.Projet1.noPermisValide;
    ...
    static void traiter(JSONObject contenu) throws ParseException {
        ...
        if (!validationCycle(contenu)) {
            ...
            for (int i = 0; i < activitesFaites.size(); i++) {
               ...
               // heure = validationHeure(heure);
                ...
                if (!ValidationCategorie(categ)) {
                    erreurs.add("L'activité " + categ + "n'appartient a aucune categorie reconnue");
                }

                if (!noPermisValide(permis)) {
                    erreurs.add("Le numeros de permis n'est pas valide");
                }

                if (!descriptionActiviteValide(description)) {
                ...
                if (ValidationCategorie(categ) && validationDate(date)) {
                    heureTotale += heure; //on increment toujours les heures
            ...
            if (nbreHeuresCours < 22) {
                erreurs.add("Il y a moins de 22 heures de cours");
            }

            if (nbreHeuresProjetDeRecherche < 3) {
                erreurs.add("Il y a moins de 3 heures de projet de recherche");
            }

            if (nbreHeuresGroupeDeDiscussion < 1) {
                erreurs.add("Il y a moins d'une heure de groupe de discution");
            }

            if ((heureTotale) < 55) {
                erreurs.add("Il manque " + (55 - heureTotale) + " heure(s) de formation pour completer le cycle.");
            }
        }
    }
}

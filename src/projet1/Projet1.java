...
public class Projet1 {
    ...
    public static void main(String[] args) throws Exception {
       ...
       /* ArrayList<String> activitesTraitees = new ArrayList<>();
        ...
        /**
         * else if ( !ValidationArchitectes.validerCycleArchitectes(cycle)) {
         * erreurs.add("Le cyle entré n'est pas supporte"); declarationValide =
         * false; } else { int heuresTransfereesCylePrecedent =
         * contenu.getInt("heures_transferees_du_cycle_precedent"); if
         * (heuresTransfereesCylePrecedent < 0) {
         * heuresTransfereesCylePrecedent = 0;
         * } else if (heuresTransfereesCylePrecedent > 7) {
         * heuresTransfereesCylePrecedent = 7; erreurs.add("Le nombre d'heures
         * transferées du cycle precedent est ramené a 7."); } JSONArray
         * activitesFaites = (JSONArray) contenu.getJSONArray("activites"); for
         ...
         * erreurs.stream().forEach((erreur) -> { listeDesErreurs.add(erreur);
         * }); contenuRetourne.put("erreurs", listeDesErreurs);
         *
         * try (FileWriter fichierDeSortie = new FileWriter(args[1])) {
         * contenuRetourne.write(fichierDeSortie); } catch (IOException e) {
         * System.out.println("Erreur d'ecriture dans le fichier specifie en
         * argument"); } }
         */
    }
}
